...
/// Standard about dialog: app name, version (from the build's pubspec), icon, and license.
/// Also exposes the bundled package licenses via the built-in "View licenses" button.
void _showAbout(BuildContext context) {
  final parts = kAppVersion.split('+');
  final version = parts.length == 2 ? '${parts[0]} (build ${parts[1]})' : kAppVersion;
  showAboutDialog(
    context: context,
    applicationName: AppConfig.current.appName,
    applicationVersion: version,
    applicationIcon: Image.asset('assets/icons/icon-512.png', width: 48, height: 48),
    applicationLegalese: '© 2026 Night Drop · AGPL-3.0-or-later',
  );
}
